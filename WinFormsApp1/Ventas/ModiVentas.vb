...
Public Class ModiVentas
    Private valorCeldaID As Object = Nothing
    Private vengoDeGuardar As Boolean = 0
    Public Sub New(VentaID As Object)
        InitializeComponent()
        valorCeldaID = VentaID
    End Sub
    ...
    Private Sub CargarDatos()
        Dim query = "SELECT * FROM ventas WHERE ID=@ID"
    ...
    Private Sub RellenarCasillas(venta As Venta)
        If venta IsNot Nothing Then
            TextBoxCliente.Text = venta.Cliente
            TextBoxFecha.Text = venta.Fecha
            TextBoxTotal.Text = venta.Total
        End If
    End Sub
    ...
    Private Sub Guardar_Click(sender As Object, e As EventArgs) Handles GuardarToolStripMenuItem.Click
        ...
        Using connection As New SqlConnection(connectionString)
            Try
                ' Abre la conexión
                connection.Open()
                Dim sqlQuery = "update ventas set IDCliente=@Cliente, Fecha=@Fecha, Total=@Total where ID=@ID"

                ' Crea un comando SQL utilizando la consulta y la conexión
                Using command As New SqlCommand(sqlQuery, connection)
                    command.Parameters.AddWithValue("@Cliente", TextBoxCliente.Text)
    ...
    Private Sub Eliminar_Click(sender As Object, e As EventArgs) Handles EliminarToolStripMenuItem.Click
        ...
        Using connection As New SqlConnection(connectionString)
            Try
                ...
                Dim sqlQuery = "DELETE FROM ventas WHERE ID=@ID"
                ...
                Using command As New SqlCommand(sqlQuery, connection)
                    ...
                    If rowsAffected > 0 Then
                        ...
                        CerrarVentana()
                    Else
                        MessageBox.Show("No se dio de baja a ninguna venta.")
                    End If
                End Using
            Catch ex As Exception
                ' Maneja cualquier error que pueda ocurrir durante la conexión o la consulta
